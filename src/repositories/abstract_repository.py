...
class TenantScopedRepository[E: Entity]:
    table: Table
    entity: type[E]
    ...
    async def update(self, entity: E) -> E:
        ...
        stmt = (
            ...
            .where(
                ...
                self.table.c.equestrian_id == data["equestrian_id"],
            )
            .values(**data)
        )
        await self.session.execute(stmt)
    ...
    async def bulk_delete(self, ids: Sequence[UUID], *, equestrian_id: UUID) -> None:
        stmt = delete(self.table).where(
            self.table.c.id.in_(ids),
            self.table.c.equestrian_id == equestrian_id,
        )
    ...
    async def get_by_slug(self, slug: str, *, equestrian_id: UUID) -> E | None:
        if "slug" not in self.table.c:
            raise AttributeError(
                f"Table {self.table.name} does not have a 'slug' column"
            )
    ...
    async def get_by_slug_or_id(
        ...
        return await self.get_by_slug(slug_or_id, equestrian_id=equestrian_id)

    async def find_by_slug(self, slug: str, *, equestrian_id: UUID) -> E | None:
        return await self.get_by_slug(slug, equestrian_id=equestrian_id)

    async def find_by_name(self, name: str, *, equestrian_id: UUID) -> E | None:
        if "name" not in self.table.c:
            raise AttributeError(
                f"Table {self.table.name} does not have a 'name' column"
            )
