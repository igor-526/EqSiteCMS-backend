...
def make_service() -> (
    tuple[NewsService, FakeNewsRepository, FakePhotoRepository, FakePhotoUrlBuilder]
):
...
async def test_ut01_create_snippet_none_html_content_autogenerated() -> None:
    ...
    news_item = await service.create(dto, user=user)
...
async def test_ut06_update_snippet_none_regenerated_from_new_content() -> None:
    ...
    updated = await service.update(existing.id, dto, user=user)
...
async def test_ut09_create_empty_name_raises_client_error() -> None:
    ...
    with pytest.raises(ClientError):
        await service.create(
            NewsCreateDto(name="", content="<p>x</p>", published_at=_FUTURE), user=user
        )
...
async def test_ut17_soft_delete_sets_is_deleted_and_deleted_at() -> None:
    ...
    await service.soft_delete(existing.id, user=user)
...
async def test_ut18_soft_delete_nonexistent_raises_client_error() -> None:
    ...
    user = make_admin_user()
    with pytest.raises(ClientError):
        await service.soft_delete(uuid4(), user=user)
...
async def test_ut19_get_cms_list_returns_deleted_records() -> None:
    ...
    items, total = await service.get_cms_list(user=user)
    assert total == 1
...
async def test_ut20_get_cms_list_status_filter_passed_to_repo() -> None:
    ...
    await service.get_cms_list(
        user=user, status=[NewsStatus.PUBLISHED, NewsStatus.SCHEDULED]
    )
...
async def test_ut25_get_cms_list_sort_passed_to_repo() -> None:
    ...
    await service.get_cms_list(user=user, sort="-published_at")
...
async def test_ut26_get_cms_list_pagination_passed_to_repo() -> None:
    ...
    await service.get_cms_list(user=user, limit=10, offset=20)
...
async def test_ut27_get_cms_list_name_filter_passed_to_repo() -> None:
    ...
    await service.get_cms_list(user=user, name="конюшня")
...
async def test_ut29_get_cms_list_no_name_match_empty_result() -> None:
    ...
    items, total = await service.get_cms_list(user=user, name="несуществующее")
    assert items == []
    assert total == 0
...
async def test_ut30_get_cms_list_snippet_filter_passed_to_repo() -> None:
    ...
    await service.get_cms_list(user=user, snippet="анонс")
...
async def test_ut31_get_cms_list_content_filter_passed_to_repo() -> None:
    ...
    await service.get_cms_list(user=user, content="подробности")
...
async def test_ut32_get_cms_list_none_name_no_filter() -> None:
    ...
    await service.get_cms_list(user=user, name=None)
...
async def test_ut33_get_cms_list_special_chars_in_name_passed_to_repo() -> None:
    ...
    await service.get_cms_list(user=user, name="test.com")
...
async def test_ut34_get_cms_list_name_and_status_combined() -> None:
    ...
    await service.get_cms_list(user=user, name="new", status=[NewsStatus.PUBLISHED])
...
async def test_ut35_get_cms_list_published_at_from_passed_to_repo() -> None:
    ...
    await service.get_cms_list(user=user, published_at_from=_PAST)
...
async def test_ut36_get_cms_list_published_at_to_passed_to_repo() -> None:
    ...
    await service.get_cms_list(user=user, published_at_to=_FUTURE)
...
async def test_ut37_get_cms_list_date_range_both_passed_to_repo() -> None:
    ...
    await service.get_cms_list(
        user=user, published_at_from=_PAST, published_at_to=_FUTURE
    )
...
async def test_ut38_get_cms_list_status_published_only() -> None:
    ...
    items, total = await service.get_cms_list(user=user, status=[NewsStatus.PUBLISHED])
    assert total == 1
...
async def test_ut39_get_cms_list_status_scheduled_only() -> None:
    ...
    items, total = await service.get_cms_list(user=user, status=[NewsStatus.SCHEDULED])
    assert total == 1
...
async def test_ut40_get_cms_list_status_deleted_only() -> None:
    ...
    items, total = await service.get_cms_list(user=user, status=[NewsStatus.DELETED])
    assert total == 1
...
async def test_ut42_get_cms_list_sort_status_passed() -> None:
    ...
    await service.get_cms_list(user=user, sort="status")
...
async def test_ut44_create_no_scope_raises_forbidden() -> None:
    ...
    with pytest.raises(ForbiddenError):
        await service.create(
            NewsCreateDto(name="Новость", content="", published_at=_FUTURE), user=user
        )
...
async def test_ut46_update_nonexistent_raises_client_error() -> None:
    ...
    user = make_admin_user()
    with pytest.raises(ClientError):
        await service.update(uuid4(), NewsUpdateDto(name="X"), user=user)
...
async def test_ut47_update_partial_untouched_fields_unchanged() -> None:
    ...
    updated = await service.update(existing.id, NewsUpdateDto(name="Новое"), user=user)
...
async def test_ut50_get_public_list_response_has_no_content_is_deleted() -> None:
    ...
    items, _ = await service.get_public_list()
    public_dto = await service.build_public_out_dto(items[0])
